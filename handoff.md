...
最終更新: 2026-05-26（Godot移行・AutoSprite課金前提へ資料更新 — 担当: Codex）／2026-05-24（Unity期 VFX語彙doc＋スプライト仕様着手／GDD腐食・量子強化ズレ報告 — 担当: Claude Code）／2026-05-23（Unity期 演出モック完成／素材生成パイプライン確定 — 担当: Claude Code）／2026-05-02（タイトル魅せ方改善／BGM軽量化／タイトルBGM起動修正）／2026-04-30（GitHub Pages導線改善／計測／SNS運用準備／キャラ選択・結果画面ポスター導入／画像軽量化）／2026-04-24（画像生成・画風基準のメモ追記）／2026-04-23（オンライン体験差修正／GitHub Pages／GDD v6.18）
...
### 2026-05-26（Godot移行・AutoSprite課金前提へ資料更新）

対象: `/Users/kazutoshi/GodotProjects/SYNTAX_WARS/`・`docs/VFX_語彙.md`・`docs/キャラスプライト仕様.md`・`docs/バトル演出設計.md`

担当エージェント: Codex

| 項目 | 内容 |
|------|------|
| エンジン方針 | Unity移行から**Godot移行**へ変更。Webプロトは引き続きバランス調整ラボ、Godot版は見せ方・演出・キャラ表示・ステージ・VFX担当 |
| ドキュメント更新 | Godotプロジェクト側の `docs/` を本命として更新。`Unity版` 表記を `Godot版` に変更し、VFX合成を `AnimatedSprite2D / Sprite2D / GPUParticles2D / CPUParticles2D` 前提に整理 |
| AutoSprite | **課金確定＝本番パイプライン採用**。ただし課金前にCodex静止画で画風・ポーズ・緑背景を確認し、NEXUS-7で完全検証してから残り5体へ展開 |
| 分業 | Codex=コンセプトアート/緑背景ポーズ/AutoSpriteプロンプト、AutoSprite=Side視点の連番化、Godot=AnimatedSprite2D配置・VFX別ノード合成・距離/カメラ/ヒット演出 |
| 位置固定 | 今回のゲームは**移動なし・立ち位置固定**。AutoSpriteには walk/接近移動を本命依頼しない。攻撃は rooted / no walking / feet stay planted を基本にし、前進感は上体の踏み込み・半歩以下の重心移動・VFX・カメラで出す |
| 注意 | 古いUnity期資料や `UnityProjects/SYNTAX_WARS_UNITY` は履歴として残っている可能性あり。今後の参照先は `/Users/kazutoshi/GodotProjects/SYNTAX_WARS/docs/` を優先 |

### 2026-05-24（Unity版 VFX語彙doc＋スプライト仕様着手／GDD腐食・量子強化ズレ報告）

対象: `/Users/kazutoshi/UnityProjects/SYNTAX_WARS_UNITY/Docs/VFX_語彙.md`・`Docs/キャラスプライト仕様.md`（新設）

担当エージェント: Claude Code

| 項目 | 内容 |
|------|------|
| VFX語彙doc | 「通常攻撃以外ほぼ全アクション/状態」を可読化優先で言語化。15項目完了（属性4・防御2・加速/スロー/回復/挑発・FAST/SLOW・腐食・オーバークロック・起爆準備）＋固有技6/通常攻撃は土台確定。正本(HTML)の実装値を逐一確認 |
| カラー設計（art-director review反映） | HEX固定の3ティア（属性/状態/キャラ固有）。キャラ固有色＝正本themeColor。**チャンネル分離**＝キャラ色("誰"＝枠/固有技/通常攻撃)とアクション色("何"＝特殊メカニクス)を分離。色覚（赤緑8%）対策で色＋形＋位置＋動きの冗長化。速度特攻はシアンRim回避でアイスブルー#2080E0へ |
| スプライト仕様doc新設 | `syntax wars 画像集/`の既存12枚（各キャラ2技）を実見し**間合い確定**：NEXUS=ハイブリッド/SHADOW=近接+投擲/NOVA=純遠隔/TITAN=近接ブルーザー/LYRA=近接バイオ/BLAZE=遠隔炎。未描画技にコンセプト付与。Codex向けアートブリーフ兼用 |
| NEXUS固有の整理 | ビーム.PNG＝プラズマカノン(強)に確定／量子ビーム(固有)＝**義眼ビーム**で差別化（収束→ロックオン→一閃） |
| 命名の決定 | 「挑発」「回復封じ」とも**正本名キープ**（Codexと語彙一致）。ツールチップで補う：挑発=「次の攻撃+30%（自己バフ）」、回復封じ=「次ターン相手の回復・挑発を封印」 |
| **⚠️ GDD腐食数値ズレ（正本側の要修正）** | 腐食ダメージ：GDD L254「LYRA 5／TITAN 3」・キャラ説明文「-3（TITAN-2）」・**実装「3／対TITAN2」** の3つが不一致。**実装が正＝3（通常）/2（対TITAN）**。腐食はLYRA-X専用でTITANは"受けにくい側"（与えない）。GDDとpassiveDescの数字を実装に合わせて要更新 |
| ⚠️ 腐食タグのデシンク（実装の癖） | 腐食ダメージは状態ベース（相手LYRA-X＋SLOW）で毎ターン発生するが、`corrode`タグはスローactionでしか立たない（HTML 5183）。速度特攻等でSLOW化した場合、ダメージは入るのにタグが出ない可能性。Godot版は表示をダメージと同条件で駆動して可読性確保（ロジックは正本のまま） |
| **⚠️ GDD量子強化ズレ（正本側の要修正）** | NEXUS量子ビームの「量子強化」：GDD L155・L605は「次ターン属性攻撃**コスト0%**」のみ記載だが、**実装は コスト0＋威力+20%**（HTML uniqueDesc／`if(hadQuantum) totalDmg*=1.2 // v6.5`）。**実装が正＝コスト0＋威力+20%**。GDDがv6.5の+20%を反映し損ねてる。要更新。※量子強化と学習(×1.8)は別ステータス（GDD L212明記） |
| **⚠️ 速度差回避の閾値ズレ（正本側の要修正）** | 強攻撃の速度差確定回避：**実装は SPEED差20以上**（HTML 4918 `defender.speed - attacker.speed >= 20`・全キャラ共通）だが、SHADOW passiveDesc/GDDは「**30以上**」と記載。**実装が正＝20**。passiveDesc/GDDを20に要更新。SHADOW回避時はゲージ+20%＋反撃必中（次の中/強/カウンター） |
| 次 | VFX残り（固有技per-move表現軸・全体演出）＋NEXUS-7をテスト1体目に未描画技をCodexコンセプトアート依頼 |

### 2026-05-23（Unity版 演出モック完成／素材生成パイプライン確定）

対象: `/Users/kazutoshi/UnityProjects/SYNTAX_WARS_UNITY/`

担当エージェント: Claude Code

| 項目 | 内容 |
|------|------|
| NEXUS-7取り込み | スプライトシート（5×5=25コマ）をスライス→idle/walkアニメ作成→1体に整理。idle=デフォルト待機再生OK |
| ステージ背景 | Codex生成 `stage_arena.png` を配置（Order -10・カメラSize≈4.7・16:9固定） |
| ライティング | URP 2Dの二色ネオン（マゼンタKEY＋シアンRim, Spot Light 2D 2灯）でキャラを浮かせる |
| 構図 | 中央リング上・Scale2。SNS用キービジュアル撮影済み |
| 画面向き | **横16:9に決定**（HTML正本は縦9:16だが演出判断で変更。バトルロジックは正本準拠） |
| パイプライン確定 | Codex=元絵/立ち絵/武器/背景/単発ポーズ生成＋autosprite操作 ／ autosprite=連番アニメ化・シート化 ／ Claude/Codex=仕様書＋Godot取り込み |
| Codexへの依頼（次） | Claudeがキャラスプライト仕様書を2部構成（①Codex向けアート指定 ②autosprite向け連番セット）で作成 → **autospriteプロンプトはCodexが作成**して生成 → 1体テスト→整合性OKで残り5体量産 |
| 要注意 | NEXUS-7のidleは差し替え予定（好みのidle基準を決めてから量産）。量産前に必ず1体で画風・頭身の整合性チェック |

#### 追記: AutoSprite量産時のキャラ差分維持

6キャラの既存9:16ポスターと技画像を確認した結果、現行ビジュアルではキャラ差は十分出ている。
今後のリスクは「全員をNEXUS-7の6頭身テンプレへ寄せすぎて、BLAZE/TITAN/LYRAなどの個性が削れること」。

量産プロンプトの基本方針:

- 画風・ライティング・質感はNEXUS-7基準で統一する
- 頭身はGodotで並べやすい6頭身前後を基準にする
- ただし全員を同じ細身黒装甲にしない
- キャラ固有シルエット、体格、色、武器/能力記号は必ず残す

キャラ別の守るべき差分:

- NEXUS-7: 黒サイバー装甲、義眼/顔面インプラント、冷たい青紫、銃/ブレード系
- BLAZE: 赤髪、炎、オレンジ/赤、荒い火力型の圧
- TITAN: 巨体、重装甲、パワー型、太い腕脚、重量感
- NOVA: 銀髪、量子/重力系SF、紫/青、細身で静かな異能感
- SHADOW: フード/忍者/暗殺者ライン、刀/影、黒紫、鋭いシルエット
- LYRA-X: 触手/異形/酸、緑の毒性、人型へ寄せすぎない。AutoSprite適用方法は別検討でもよい

結論:

AutoSprite用プロンプトは「全員NEXUS-7風にする」ではなく、
**シリーズ画風だけ揃えて、キャラ固有シルエットは守る** 方針で作る。
